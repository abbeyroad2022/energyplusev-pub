...
gulp.task('sass', () => {
	return new Promise(resolve => {
      ...
      .src(
        [
          dir.src_scss + "/*.scss",
          "!" + dir.src_scss + "/config.scss", // 셋팅파일 컴파일 제외
        ],
        { allowEmpty: true }
      )
...
gulp.task('accordion-js', () => {
	return new Promise(resolve => {
		gulp.src([
			'node_modules/accordion-js/dist/*.js',
			'node_modules/accordion-js/dist/*.css'
		], {allowEmpty: true})
		.pipe(gulp.dest(dir.dist_lib + '/accordion-js/'))
		resolve();
	});
});
...
gulp.task('plugin', () => {
	return new Promise(resolve => {
		gulp.src(dir.src_plugin + '/**/*', {allowEmpty: true})
		.pipe(gulp.dest(dir.dist_lib))
		resolve();
	});
});
...
gulp.task('watch', () => {
	return new Promise(resolve => {
		...
		gulp.watch(dir.src_scss + '/**/*.scss', gulp.series(['sass']));
